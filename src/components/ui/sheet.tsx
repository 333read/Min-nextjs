...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      <SheetPrimitive.Close className="absolute left-4 top-4 rounded-full opacity-70  hover:text-white hover:bg-theme-color focus:outline-none focus:ring-2 focus:ring-theme-color/85 focus:ring-offset-2">
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn("text-sm text-muted-foreground px-3", className)}
    {...props}
  />
))
